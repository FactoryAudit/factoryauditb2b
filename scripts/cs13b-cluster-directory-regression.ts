 ...
 *
 * 覆盖：
 *   A 冻结层：9 语字典 clusters.allCountries 到位 + en 叶子数 2940 + 8 处门禁常量同源
...
console.log("--- A 冻结层 ---");
{
  ...
  ok("A1 en 字典叶子数 = 2940（新增 1 键）", enLeaves === 2940, `实际 ${enLeaves}`);
  ...
  const gates: Array<[string, string]> = [
    ...
    ["scripts/cs13-supplier-seo-regression.ts", "EN_LEAF_COUNT = 2940"],
    ["scripts/cs16-supplier-mgmt-regression.ts", "EN_LEAF = 2940"],
    ["scripts/cs17-commerce-regression.ts", "EN_LEAF = 2940"],
    ["scripts/cs20-supplier-report.ts", "EN_LEAF = 2940"],
    ["RELEASE-RULES.md", "**2940**"],
  ];
  for (const [f, pat] of gates) {
    const src = read(f);
    // 历史 changelog 行（含 "→"）里的旧数字不算残留
    const stale = src.split("\n").some((l) => !l.includes("→") && l.includes("2939"));
    ok(`A5 ${f} 常量 → 2940 且无残留 2939`, src.includes(pat) && !stale);
  }
}
