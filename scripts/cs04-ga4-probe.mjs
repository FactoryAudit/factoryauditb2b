 ...
 *   - page_view_group 在首次加载时不被投递（触发早于 gtag.js 就绪），属 P2 已知项。
 *   - 别在页面里注入任何 dataLayer 垫片后再断言投递：那会让 fallback 路径
 *     （dataLayer.push({event})）也被记录，而 gtag.js 并不发送该形态，导致假结论。
 */
...
ws.addEventListener("message", (e) => {
  ...
  const u = new URL(url);
  // GA4 可能把参数放在 query，也可能放在 POST body —— 两处都要读，否则会漏事件
  let events = u.searchParams.getAll("en");
  if (post) {
    for (const mm of post.matchAll(/(?:^|&)en=([^&]*)/g)) events.push(decodeURIComponent(mm[1]));
  }
  if (!events.length) events = ["(none)"];
  reqs.push({ phase, host: u.host, tid: u.searchParams.get("tid") || "(in body)", events });
});
  ...
  JSON.stringify(
    {
      ...
      assertions: {
        "page_view 每页恰好 1 次（3 页 → 期望 3）": count("page_view"),
      ...
      perRequest: reqs.map((r) => `${r.phase} -> [${r.events.join(",")}]`),
    },
    null,
    2
  )
