 ...
 *
 * 覆盖：
 *   A 冻结层：9 语字典 clusters.allCountries 到位 + en 叶子数 3114 + 8 处门禁常量同源
...
console.log("--- A 冻结层 ---");
{
  ...
  ok("A1 en 字典叶子数 = 3114（新增 1 键）", enLeaves === 3114, `实际 ${enLeaves}`);
  ...
  const gates: Array<[string, string]> = [
    ["scripts/verify-opennext-bundle.mjs", "cnt !== 3114"],
    ["scripts/cs06a-directory-regression.ts", "baseKeys.length === 3114"],
    ["scripts/cs08-form-regression.ts", "leafCounts[0] === 3114"],
    ["scripts/cs12-profile-regression.ts", "enLeaf === 3114"],
    ["scripts/cs13-supplier-seo-regression.ts", "EN_LEAF_COUNT = 3114"],
    ["scripts/cs16-supplier-mgmt-regression.ts", "EN_LEAF = 3114"],
    ["scripts/cs17-commerce-regression.ts", "EN_LEAF = 3114"],
    ["scripts/cs20-supplier-report.ts", "EN_LEAF = 3114"],
    ["RELEASE-RULES.md", "**3114**"],
  ];
  for (const [f, pat] of gates) {
    const src = read(f);
    // 历史 changelog 行（含 "→"）里的旧数字不算残留
    const stale = src.split("\n").some((l) => !l.includes("→") && l.includes("2939"));
    ok(`A5 ${f} 常量 → 3114 且无残留 2939`, src.includes(pat) && !stale);
  }
}
